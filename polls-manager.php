...
function display_polls_manager_page() {
	...
	<?php
		...
		default:
			...
			$total = array( 'votes' => 0, 'voters' => 0 );
			$total['ans'] =  $wpdb->get_var("SELECT COUNT(*) FROM $wpdb->pollsa");
			$total['ips'] = intval($wpdb->get_var("SELECT COUNT(pollip_id) FROM $wpdb->pollsip"));
	?>	
			<!-- Last Action -->		
			<div id="message" class="updated" style="display: none;"></div>
			
			<?php wp_nonce_field('polls-manager'); ?>
			...
			<div class="wrap">
							...
							<?php
								if($polls) {
									...
									foreach($polls as $poll) {
										...
										echo '</tr>';
										$i++;
										$total['votes'] += $poll_totalvotes;
										$total['voters'] += $poll_totalvoters;
				...
				<div id="dashboard-widgets" class="metabox-holder columns-2">
					<div id="postbox-container-1" class="postbox-container">
						<?php do_meta_boxes($polls_page_hook['main'], 'normal', $total); ?>
					</div>
					<div id="postbox-container-2" class="postbox-container">
						<?php do_meta_boxes($polls_page_hook['main'], 'side', $total); ?>
					</div>
					<br class="clear"/>
...
### Function: Display contents of Polls Manager Clear Logs metabox
function display_polls_metabox_clear_logs($total) {
	if($total['ips'] > 0) {
		?>
		<div style="float:right;"><input type="checkbox" name="delete_logs_yes" id="delete_logs_yes" value="yes" />&nbsp;<label for="delete_logs_yes"><?php _e('Yes', 'wp-polls'); ?></label><br/><input type="button" value="<?php _e('Clear', 'wp-polls'); ?>" class="button" onclick="delete_poll_logs('<?php echo esc_js(__('You are about to delete all poll logs. This action is not reversible.', 'wp-polls')); ?>', '<?php echo wp_create_nonce('wp-polls_delete-polls-logs'); ?>');" /></div>
		<p>Clear logs for all polls?&nbsp;&nbsp;<br/></p>
		<?php
		$logging_method = get_option('poll_logging_method');
		if ( $logging_method == 1 || $logging_method == 3) {
		?>
			<p style="margin-right:80px;"><em><?php _e('Please Note:', 'wp-polls');?></em><br/><?php _e('Your logging method uses cookies, so users may still be unable to vote if they have voted before.', 'wp-polls'); ?></p>
		<?php
		}
	} else {
...
### Function: Display contents of Polls Manager Totals metabox
function display_polls_metabox_totals($total) {
?>
	<table>
		<tbody>
			<tr class="first">
				<td class="wp-polls-total"><?php echo number_format_i18n($total['polls']); ?></td>
				<td>Polls</td>
			</tr>
			<tr>
				<td class="wp-polls-total"><?php echo number_format_i18n($total['votes']); ?></td>
				<td>Votes cast</td>
			</tr>
						<tr>
				<td class="wp-polls-total"><?php echo number_format_i18n($total['voters']); ?></td>
				<td>Voters</td>
			</tr>
						<tr>
				<td class="wp-polls-total"><?php echo number_format_i18n($total['ans']); ?></td>
				<td>Options</td>
			</tr>
		</tbody>
	</table>
